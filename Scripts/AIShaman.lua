...
function AIShaman:handleShamanCombat()
  ...
  for i=0, 7 do 
    ...
    if (tempShaman ~= nil) then
      if (tempShaman.Owner ~= self.tribe) then
        ...
        if (isAlly == false or tableLength(self.allies) < 1) then
  ...
  for i, t in pairs(enemyShamans) do
    ...
    if (self.shaman ~= nil) then
      local dist = get_world_dist_xyz(self.shaman.Pos.D3, t.Pos.D3)
      if (t.Owner ~= self.tribe and get_world_dist_xyz(self.shaman.Pos.D3, t.Pos.D3) < self.aggroRange*2) then
        self.target = t
...
function AIShaman:resetSpellsMissed()
  ...
  if (get_world_dist_xyz(self.shaman.Pos.D3, self.target.Pos.D3) > self.aggroRange) then
    self.enemyShamanNearby = 0
    self.lightsMissed = 0
    self.blastsFailed = 0
    self.target = nil
...
function AIShaman:handleBlastTrick()
  ...
  if (self.flyingDuration > 7 and self.flyingDuration < 9 and self.blastTrickAllowed == 1 and self.shaman.State ~= S_PERSON_ELECTROCUTED and get_world_dist_xyz(self.shaman.Pos.D3, self.target.Pos.D3) < 4000 and self.blastTrickDelay == 0 and self.smartCastsBlast < self.maxSmartCastsBlast and self.wasInBlastTrickRange == 1) then
    ...
    GIVE_MANA_TO_PLAYER(self.tribe, self.manaCostBlast * -1)
    return false
  elseif (self.flyingDuration > 7 and self.flyingDuration < 9 and self.blastTrickAllowed == 1 and self.shaman.State ~= S_PERSON_ELECTROCUTED and self.blastTrickDelay == 0 and self.smartCastsBlast < self.maxSmartCastsBlast) then --Blast myself if enemy is too far
    createThing(T_SPELL, M_SPELL_BLAST, self.shaman.Owner, self.shaman.Pos.D3, false, false)
    self.spellDelay = 24 + G_RANDOM(11)
    self.blastTrickDelay = 12
    self.smartCastsBlast = self.smartCastsBlast + 1
...
function AIShaman:dodgeSpellsAndFollowEnemyShaman()
  --Dodge lightning if needed, otherwise only try to dodge blast
    if (self.dodgeLightning == 1 and self.shaman.State ~= S_PERSON_FIGHT_PERSON_2) then
      ...
      elseif (get_world_dist_xyz(self.target.Pos.D3, self.shaman.Pos.D3) < self.aggroRange and self.followEnemyDelay == 0 and MANA(self.tribe) > self.manaCostLightning * 2 and self.blastAllowed == 1) then --If enemy in aggro range and I don't have enough mana for 2 lightning bolts walk to them,  but only if I can use blast
        local c2d = Coord2D.new()
        ...
        self.dodgeDelay = 28
      elseif (everyPow(self.dodgeDelay, 1) and get_world_dist_xyz(self.target.Pos.D3, self.shaman.Pos.D3) < 6000 + self.target.Pos.D3.Ypos*3) then
        SearchMapCells(CIRCULAR, 0, 0, 4, world_coord2d_to_map_idx(self.shaman.Pos.D2), function(me)
          if (is_map_elem_land_or_coast(me) > 0) then
            local c2d = Coord2D.new()
            map_ptr_to_world_coord2d(me, c2d)
      ...
      elseif (get_world_dist_xyz(self.target.Pos.D3, self.shaman.Pos.D3) < self.aggroRange and self.followEnemyDelay == 0) then --If enemy in aggro range walk towards them
        local c2d = Coord2D.new()
        local m = MapPosXZ.new()
        m.Pos = world_coord3d_to_map_idx(self.target.Pos.D3)
        map_xz_to_world_coord2d(m.XZ.X, m.XZ.Z, c2d)
    ...
    elseif (self.dodgeBlast == 1 and self.shaman.State ~= S_PERSON_FIGHT_PERSON_2) then   --Dodging blasts
      ...
      elseif (get_world_dist_xyz(self.target.Pos.D3, self.shaman.Pos.D3) < self.aggroRange and self.followEnemyDelay == 0) then
        local c2d = Coord2D.new()
        local m = MapPosXZ.new()
        m.Pos = world_coord3d_to_map_idx(self.target.Pos.D3)
        map_xz_to_world_coord2d(m.XZ.X, m.XZ.Z, c2d)
...
function AIShaman:castBlastOnShaman()
  ...
  if (get_world_dist_xyz(self.shaman.Pos.D3, self.target.Pos.D3) < 3800 and self.blastsFailed >= self.blastsFailedBeforeAngry and self.spellDelay == 0 and MANA(self.tribe) > self.manaCostBlast and is_thing_on_ground(self.shaman) == 1 and self.targetThatIsInAir ~= self.target and self.blastAllowed == 1) then
    if (get_world_dist_xyz(self.shaman.Pos.D3, self.target.Pos.D3) < 2500) then
      self.blastsFailed = 0
      local blast = createThing(T_SPELL, M_SPELL_BLAST, self.shaman.Owner, self.target.Pos.D3, false, false)
      blast.u.Spell.TargetThingIdx:set(self.target.ThingNum)
      ...
      if (self.blastBadMouthed == 0) then
        log_msg(self.tribe, self.tribeName..": Fight me without that cheesy stuff, human")
...
function AIShaman:counterGhostArmies()  
  if (self.ghostsAllowed == 1) then
    SearchMapCells(CIRCULAR, 0, 0, 6, world_coord3d_to_map_idx(self.shaman.Pos.D3), function(me) 
      me.MapWhoList:processList(function(t)
        if (t.Type == T_PERSON) then
          if (t.Owner ~= self.tribe and isAlly == false and get_world_dist_xyz(self.shaman.Pos.D3, t.Pos.D3) < 3100 + self.shaman.Pos.D3.Ypos*3 and t.Flags2 & TF2_THING_IS_A_GHOST_PERSON ~= 0) then
            --Destroy ghost armies near me with ghost armies or blast if ghost armies are not ready
            if (get_world_dist_xyz(self.shaman.Pos.D3, t.Pos.D3) < 3000 + self.shaman.Pos.D3.Ypos*3 and self.ghostsAllowed == 1 and MANA(self.tribe) > self.manaCostGhostArmy and self.spellDelay == 0 and self.enemyCastDelay > 25 and self.enemyCastDelay < 36 and self.ghostsSpecialDelay == 0) then
              if(is_thing_on_ground(self.shaman) == 1) then
            ...
            elseif (get_world_dist_xyz(self.shaman.Pos.D3, t.Pos.D3) < 1500 + self.shaman.Pos.D3.Ypos*3 and self.blastAllowed == 1 and MANA(self.tribe) > self.manaCostBlast and self.spellDelay == 0 and self.enemyCastDelay > 25 and self.enemyCastDelay < 36) then
              if(is_thing_on_ground(self.shaman) == 1) then
                local blast = createThing(T_SPELL, M_SPELL_BLAST, self.shaman.Owner, t.Pos.D3, false, false)
                blast.u.Spell.TargetThingIdx:set(t.ThingNum)
            ...
            elseif (get_world_dist_xyz(self.shaman.Pos.D3, t.Pos.D3) < 700 and self.ghostsAllowed == 1 and MANA(self.tribe) > self.manaCostGhostArmy and self.spellDelay == 0 and self.ghostsSpecialDelay == 0) then
              if(is_thing_on_ground(self.shaman) == 1) then
                local ghosts = createThing(T_SPELL, M_SPELL_GHOST_ARMY, self.shaman.Owner, t.Pos.D3, false, false)
                ghosts.u.Spell.TargetThingIdx:set(t.ThingNum)
                self.spellDelay = 12
...
function AIShaman:checkSpellDelay()
  if (self.flyingDuration > 0) then
    if (self.shaman ~= nil) then
      if (self.flyingDuration > 0 and is_thing_on_ground(self.shaman) == 1) then
        self.flyingDuration = 0
      end
    end
  end
  if (self.shaman ~= nil ) then
    if (is_thing_on_ground(self.shaman) == 0) then
  ...
  if (self.blastsFailed >= 1 and self.shaman.State == S_PERSON_SPELL_TRANCE) then
    self.blastsFailed = 0
  end
